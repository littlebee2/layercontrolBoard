...
rt_inline int _can_poll_tx(struct rt_can_device *can, const struct rt_can_msg *data, int msgs)
{
	int size;

	size = msgs;

	if(can->ops->sendmsg(can, data, 0) != RT_EOK)
	{
                              ...
                              rt_size_t         size)
{
    ...
    if ((dev->open_flag & RT_DEVICE_FLAG_INT_TX) && (dev->ref_count > 0))
    {
		...
		{
			return _can_poll_tx(can, buffer, size);   // add  ·ÇÖÐ¶Ï·¢ËÍ
		}
    return 0;
}
